...
在上面的模板中，我們爲每個學科添加了一個超鏈接，點擊超鏈接可以查看該學科的講師信息，爲此需要再編寫一個視圖函數來處理查看指定學科老師信息。
...
設計視圖函數`praise_or_criticize`來支持“好評”和“差評”功能，該視圖函數通過Django封裝的JsonResponse類將字典序列化成JSON字符串作爲返回給瀏覽器的響應內容。
