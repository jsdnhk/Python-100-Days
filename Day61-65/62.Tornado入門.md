...
在啓動Web應用時，如果沒有指定端口，將使用`define`函數中設置的默認端口8000，如果要指定端口，可以使用下面的方式來啓動Web應用。
...
Tornado的模板語法與其他的Web框架中使用的模板語法並沒有什麼實質性的區別，而且目前的Web應用開發更倡導使用前端渲染的方式來減輕服務器的負擔，所以這裏我們並不對模板語法和後端渲染進行深入的講解。
